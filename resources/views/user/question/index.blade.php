...
<div class="main-wrap">
  {!! Form::open(['route' => 'question.index', 'method' => 'GET', 'id' => 'category-form']) !!}
    ...
    <div class="category-wrap">
      <div class="btn all @if(empty($inputs['tag_category_name'])) selected @endif" id="0">all</div>
      @foreach ($tagCategories as $tagCategory)
        <div class="btn {{ $tagCategory->name }} @if(isset($inputs['tag_category_name']) && $tagCategory->id == $inputs['tag_category_name']) selected @endif" id="{{ $tagCategory->id }}">{{ $tagCategory->name }}</div>
      @endforeach
      {!! Form::hidden('tag_category_name', null, ['id' => 'category-val']) !!}
    </div>
  {!! Form::close() !!}
